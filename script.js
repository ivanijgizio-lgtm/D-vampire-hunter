(function() {
  ...
  let game = {
    ...
    player: { army: { location: 13, power: 100 }, elites: 5 },
    ai: { armies: [{ location: 15, power: 100 }], elites: 1 }
  };
  ...
  function moveArmy(targetId) {
    ...
    if (prov.siegeBy === 'player') { addLog('⚔️ Уже осаждаем. Используйте штурм.'); return; }
    ...
    prov.siegeBy = 'player';
    prov.siegeTurns = 1;
    game.player.army.location = targetId;  // перемещаем армию
  ...
  function assault() {
    ...
    if (success) {
      prov.owner = 'player'; prov.garrison = 20; prov.elite = 0; prov.siegeTurns = 0; prov.siegeBy = null;
  ...
  function cancelSiege() {
    const prov = getProv(game.player.army.location);
    if (!prov || prov.siegeBy !== 'player') { addLog('Нет активной осады.'); return; }
    prov.siegeTurns = 0; prov.siegeBy = null;
    game.player.army.location = 13; // возврат в Трансильванию
    ...
    game.provinces.forEach(p => {
      if (p.siegeBy === 'player') {
        ...
        if (p.siegeTurns >= 3 || p.garrison <= 0) {
          p.owner = 'player';
          p.garrison = 10; p.elite = 0; p.siegeTurns = 0; p.siegeBy = null;
    ...
    game.ai.armies.forEach(army => {
      ...
      if (targets.length) {
        ...
        if (army.power > defPower * 0.6) {
          if (fight(army.power + game.ai.elites * 5, defPower, targetId, false)) {
            tProv.owner = 'ai';
            tProv.garrison = 20; tProv.elite = 0;
  ...
  function drawProvince(prov) {
    ...
    if (!game.revealed.has(prov.id)) {
      ctx.fillStyle = '#2a2319'; ctx.fill(); ctx.strokeStyle = '#1a130c'; ctx.lineWidth=2; ctx.stroke();
      return;
    }
    ...
    ctx.font='bold 11px "Courier New"'; ctx.fillStyle='#fdf5e6';
    ctx.shadowColor='#000'; ctx.shadowBlur=4;
    ctx.fillText(prov.name, prov.centroid.x-22, prov.centroid.y-6);
    ctx.shadowBlur=0;
    const cx = prov.centroid.x, cy = prov.centroid.y;
    if (prov.buildings.includes('cathedral')) drawCathedral(cx-8, cy-18);
    if (prov.buildings.includes('cemetery')) drawCemetery(cx-6, cy+8);
    if (prov.elite>0) { ctx.fillStyle='#fff'; ctx.font='bold 12px monospace'; ctx.fillText(prov.elite, cx+12, cy-16); }
    if (prov.garrison>0) { ctx.fillStyle='#ccc'; ctx.font='9px monospace'; ctx.fillText(prov.garrison, cx-22, cy+14); }
    if (prov.siegeBy) { ctx.fillStyle='#ff6600'; ctx.font='bold 14px monospace'; ctx.fillText('⚔️', cx-10, cy-24); }
  }

  function drawCathedral(x, y) {
    ctx.fillStyle = '#b89b7b'; ctx.fillRect(x, y+4, 16, 10);
    ctx.fillStyle = '#d4af37'; ctx.fillRect(x+6, y-2, 4, 10); ctx.fillRect(x, y+2, 16, 3);
  }
  function drawCemetery(x, y) {
    ctx.fillStyle = '#5a5a5a'; ctx.fillRect(x+2, y, 12, 8);
    ctx.fillStyle = '#2f2f2f'; ctx.fillRect(x, y-2, 2, 6); ctx.fillRect(x+12, y-2, 2, 6);
  }
  function drawUnits() {
  ...
  function drawVampireKnight(x, y) {
    ctx.fillStyle='#1a1a1a'; ctx.fillRect(x, y+2, 14, 16);
    ctx.fillStyle='#8b0000'; ctx.fillRect(x-2, y+4, 18, 6);
    ctx.fillStyle='#d4af37'; ctx.fillRect(x+2, y-4, 10, 6);
    ctx.fillStyle='#ff0000'; ctx.fillRect(x+8, y-2, 3, 3);
  }
  function drawInquisitor(x, y) {
    ctx.fillStyle='#e0e0e0'; ctx.fillRect(x, y+2, 14, 16);
    ctx.fillStyle='#1e3a8a'; ctx.fillRect(x-2, y+4, 18, 6);
    ctx.fillStyle='#ffd700'; ctx.fillRect(x+2, y-4, 10, 6);
    ctx.fillStyle='#1e3a8a'; ctx.fillRect(x+6, y-2, 2, 8); ctx.fillRect(x+2, y+2, 10, 2);
  }
  function draw() { drawMap(); game.provinces.forEach(drawProvince); drawUnits(); if (game.night) { ctx.fillStyle='rgba(10,10,30,0.25)'; ctx.fillRect(0,0,1000,700); } }
  ...
  canvas.addEventListener('click', e => {
    ...
    for (let p of game.provinces) {
      ...
      if (mx >= Math.min(...xs) && mx <= Math.max(...xs) && my >= Math.min(...ys) && my <= Math.max(...ys)) {
        ...
        else addLog('🌫️ Территория скрыта туманом.');
        break;
      }
    }
  });

...
// ================= МУЗЫКА И ТУТОРИАЛ =================
function playBackgroundMusic() { const bgm = document.getElementById('bgm'); if (bgm) { bgm.volume = 0.4; bgm.load(); bgm.play().catch(() => {}); } }
...
function getLordBonus() {
    let attackBonus = 0;
    game.player.lords.forEach(l => { if (l.battles >= 2 && l.battles < 5) attackBonus += 0.1; else if (l.battles >= 5) attackBonus += 0.2; });
    return attackBonus;
}
function processLordsAfterBattle(isVictory, isAttacker) {
    ...
    game.player.lords.forEach(l => {
        if (isVictory) { l.battles += 1; aliveLords.push(l); } 
        else { log(`💀 Лорд "${l.name}" погиб в бою!`, 'player'); }
    });
...
function calcArmyPower(army, isPlayer) {
    ...
    let generalBonus = 1;
    if (isPlayer) { if (game.player.techs.militaryReform) generalBonus += 0.1; generalBonus += getLordBonus(); } 
    else { generalBonus = 1.2; }
...
function checkCaptureRequirements(attackerSide, targetProv) {
    ...
    if (getTotalTroops(army) < 1) return log('❌ В армии нет войск!', 'system') && false;
...
function executeBattle(attackerSide, targetProv) {
    ...
    if (attIsPlayer && game.player.techs.necromancy && defenderLosses > 0) { game.player.blood += Math.floor(defenderLosses / 2); log(`💀 Некромантия: +${Math.floor(defenderLosses / 2)} крови.`, 'player'); }
...
function buildStructure(type) {
    ...
    if (type === 'dark_temple') { prov.loyalty = Math.min(100, prov.loyalty + 3); prov.support.player = Math.min(100, prov.support.player + 5); log(`🕯️ Храм Тьмы возведен!`, 'player'); } 
    else if (type === 'dungeon') { prov.loyalty = Math.max(0, prov.loyalty - 5); prov.support.player = Math.min(100, prov.support.player + 10); log(`⛓️ Тюрьма возведена!`, 'player'); } 
    ...
    else if (type === 'citadel') { game.player.hasCitadel = true; log(`🏰 Цитадель построена! Открыт найм Сборщиков душ.`, 'player'); } 
    ...
    else if (type === 'barracks') { log(`🏗️ Построены Казармы Lv1!`, 'player'); }
...
function recruitTroops(type) {
    ...
    if (type === 'lord') {
        ...
        if (!hasDarkTemple) return log('❌ Постройте Храм Тьмы (20🪙)!', 'system');
        if (game.player.gold < 10) return log('❌ Нужно 10 золота для призыва Лорда.', 'system');
        game.player.gold -= 10; addNewLordToPlayer(); game.player.ap -= 1; updateUI(); return;
    }
